...
export function Footer() {
  return (
    <footer className="bg-bg-secondary border-t border-border relative overflow-hidden">
      {/* Subtle glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-accent/[0.03] rounded-full blur-[100px] pointer-events-none" />
          ...
          <div>
            ...
            <ul className="space-y-3">
              {["Web Applications", "Mobile Apps", "AI & Automation", "Integrations & Bots"].map((s) => (
                <li key={s}><Link href="/services" className="footer-link text-text-muted hover:text-text-primary transition-colors text-sm">{s}</Link></li>
              ))}
            </ul>
          </div>
          <div>
